...
class BenefitRequest(models.Model):
    ...
    def _register_change_state(self, vals):
        vals.update(
            {"last_change_state": fields.Date.today(), "last_state": _(self.state)}
        )
        ...
        message = _("State changed from <b>%s</b> to <b>%s</b>") % (
            old_state,
            new_state,
        )
    ...
    def _log_school_benefits_changes(self, commands):
        ...
        for command in commands:
            ...
            if command[0] == 0:  # Create
                ...
                if benefit.exists():
                    message = (
                        _('<b>School Benefit</b> "%s" updated')
                        % benefit.name_get()[0][1]
                    )
                    self.message_post(body=message, message_type="notification")
                ...
                if benefit.exists():
                    message = (
                        _('<b>School Benefit</b> "%s" removed')
                        % benefit.name_get()[0][1]
                    )
                    self.message_post(body=message, message_type="notification")
                ...
                if benefit.exists():
                    message = (
                        _('<b>School Benefit</b> "%s" unlinked')
                        % benefit.name_get()[0][1]
                    )
                    self.message_post(body=message, message_type="notification")
    ...
    def _message_get_suggested_recipients(self):
        ...
        recipients[self.id].append(
            (self.partner_id.id, self.partner_id.name, "Solicitante")
        )
        return recipients
